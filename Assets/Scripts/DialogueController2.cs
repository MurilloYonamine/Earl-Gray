...
public class DialogueController2 : MonoBehaviour
{
    ...
    [SerializeField] private Image componenteImagem2;
    ...
    [SerializeField] private List<Sprite> imagens2;
    ...
    void Start()
    {
        ...
        componenteImagem2.gameObject.SetActive(false);
        index = 0;
        ComecarDialogo();
    }
    ...
    void ComecarDialogo()
    {
        if (index >= falas.Count || index >= nomes.Count || index >= imagens.Count || index >= imagens2.Count)
        {
    ...
    void AtualizarImagem()
    {
        if (imagens[index] != null)
        {
            ...
            componenteImagem2.sprite = imagens2[index];
            componenteImagem.gameObject.SetActive(true);
            componenteImagem2.gameObject.SetActive(true);
        }
        else
        {
            componenteImagem.gameObject.SetActive(false);
